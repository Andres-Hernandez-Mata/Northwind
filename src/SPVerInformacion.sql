CREATE PROCEDURE VerInformacion
AS
BEGIN	
	...
	ALTER TABLE lsbd_EncryMask  
	ALTER COLUMN infoMaskDireccion DROP MASKED;

	ALTER TABLE lsbd_EncryMask  
	ALTER COLUMN infoMaskTelefono DROP MASKED;

	ALTER TABLE lsbd_EncryMask  
	ALTER COLUMN infoMaskEmail DROP MASKED;

	SELECT id, CONVERT(VARCHAR(MAX), DECRYPTBYPASSPHRASE('LSTI', infoEncryNombre)) 
